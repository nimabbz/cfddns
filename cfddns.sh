...
JSON_EOF
    ...
    if echo "$UPDATE_RESULT" | grep -q '"success":true'; then
        echo "$(date '+%Y-%m-%d %H:%M:%S') - [SUCCESS] DNS Record updated to $NEW_IP ($EXEC_MODE)" >> "$LOG_FILE"
    else
        local CF_ERROR_CODE=$(echo "$UPDATE_RESULT" | jq -r '.errors[0].code' 2>/dev/null)
        local HUMAN_ERROR="Unknown Cloudflare API Error."
        case "$CF_ERROR_CODE" in
            7003) HUMAN_ERROR="Invalid Zone ID or Record ID." ;;
            10001) HUMAN_ERROR="Authentication failed. Invalid API Token." ;;
            81053) HUMAN_ERROR="DNS Record already exists with this IP." ;;
            *) HUMAN_ERROR="Cloudflare API Error Code: $CF_ERROR_CODE" ;;
        esac
        echo "$(date '+%Y-%m-%d %H:%M:%S') - [FAILED] $HUMAN_ERROR ($EXEC_MODE)" >> "$LOG_FILE"
    fi
}
...
if [ -z "$CURRENT_IP" ]; then echo "$TIMESTAMP - [ERROR] Server could not fetch its own IP address." >> "$LOG_FILE"; exit 1; fi

if [ "$CURRENT_IP" != "$CF_IP" ]; then
    echo "$TIMESTAMP - [INFO] IP Change Detected! Old: ${CF_IP:-None} | New: $CURRENT_IP" >> "$LOG_FILE"
    update_cf_ip "$CURRENT_IP"
elif [ "$EXEC_MODE" == "MANUAL" ]; then
    echo "$TIMESTAMP - [INFO] Server IP ($CURRENT_IP) is unchanged. No update required." >> "$LOG_FILE"
fi
EOF
